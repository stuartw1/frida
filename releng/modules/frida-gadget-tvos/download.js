...
async function download() {
  const response = await httpsGet(`https://github.com/frida/frida/releases/download/${gadget.version}/frida-gadget-${gadget.version}-tvos-universal.dylib.gz`);
...
async function pruneOldVersions() {
  ...
  for (const name of await readdir(gadgetDir)) {
    if (name.startsWith('frida-gadget-') && name.endsWith('-tvos-universal.dylib') && name !== currentName) {
